...
export class WarningAdminController {
  ...
  @Get()
  @ApiOperation({ summary: 'List all warnings' })
  async findAll(@Query() query: WarningQueryAdminDto) {
    return this.service.findAll(query);
  }

  @Get(':id')
  @ApiOperation({ summary: 'Get warning details' })
  async findOne(@Param('id', ParseIntPipe) id: number) {
    return this.service.findOne(id);
  }

  @Post()
  @ApiOperation({ summary: 'Issue warning to user' })
  async create(
  ...
  @Patch(':id/resolve')
  @ApiOperation({ summary: 'Remove warning' })
  async resolve(@Param('id', ParseIntPipe) id: number) {
    return this.service.resolve(id);
  }

  @Patch(':id/cancel')
  @ApiOperation({ summary: 'Revoke warning' })
